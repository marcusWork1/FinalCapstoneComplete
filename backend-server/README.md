# Final Capstone Starter Backend Server Project

## Database

Inside the `final-capstone-team-project-name/backend-server/database` directory, you'll find an executable Bash script (`.sh` file) and several SQL scripts (`.sql` files). These can be used to build and rebuild a PostgreSQL database for the capstone project.

From a terminal session, execute the following commands:

```
cd final-capstone-team-project-name/backend-server/database
./create.sh
```
...
There is no reason to run these scripts individually.  The `create.sh` runs them for you.  They are provided for you to use a reference for any SQL you may need to write. ***DO NOT MAKE ANY CHANGES TO THE SQL IN ANY OF THESE FILES***
### Database users
...
Most of the functionality related to Security is located in the `src/main/java/com/techelevator/aDO_NOT_CHANGE_ANYTHING_IN_THIS_PACKAGE/security` package. You shouldn't have to modify anything here, but feel free to go through the code if you want to see how things work.

### Authentication Controller

There is a single controller in the `com.techelevator.controller.authentication` package called `AuthenticationController.java`.
...
**Don't worry about writing any tests or using the one's provided.  Verify your project by running it and ensuring the application behaves as expected and any exceptions are handled.**
